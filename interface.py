...
class InterfaceApp(tk.Tk):
    def __init__(self):
        ...
        self.btn_calculate.grid(row=2, column=0, padx=15, pady=(20, 20), sticky="ew")
    ...
    def calculate(self):
        ...
        for resultant in self.resultants:
            self.frame_illustration.apply_force(resultant.distance, resultant.teta, 'R')
            self.frame_illustration.apply_suport(resultant.distance)

            self.frame_resultants.create_force_input(resultant, 'R')

        for force in self.forces:
            self.frame_illustration.apply_force(force.distance, force.teta, 'F')
